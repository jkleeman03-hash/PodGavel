// Regenerates just the ad-read.mp3 for the homepage voice demo, reusing an
// already-cloned voiceId (skips re-cloning).
// Usage: npx tsx --env-file=.env.local scripts/regenerate-ad-read.ts <voiceId>
...
async function main() {
  ...
  if (!voiceId) {
    console.error("Usage: npx tsx scripts/regenerate-ad-read.ts <voiceId>");
    process.exit(1);
  }

  console.log(`Generating ad read with voice ${voiceId} (conversational settings)...`);
  const audio = await generateSpeech({ voiceId, text: AD_COPY });
